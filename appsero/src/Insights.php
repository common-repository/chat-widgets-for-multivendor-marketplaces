...
class Insights
{
	...
	public function __construct($client, $name = null, $file = null)
	{
		...
		if (is_object($client) && is_a($client, 'Appsero\CWMM\Client')) {
			$this->client = $client;
		}
	}
	...
	protected function init_common()
	{
		...
		// uninstall reason
		add_action('wp_ajax_' . $this->client->slug . '_submit-uninstall-reason', array(
			$this,
	...
	public function sync_fluent_contact()
	{
		...
		$fluent_data = [
			...
			'email'      => $data['admin_ema il'],
		];

		$product = new \WPPOOL\Plugin('chat_widgets_for_multivendor_marketplaces', $fluent_data);
